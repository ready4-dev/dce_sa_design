...
```{r eval=FALSE}
launch_survey_preview(dce_design_ls, set_idx_1L_int = 1L, block_1L_int = 1L)
```

```{r eval=FALSE}
launch_survey_preview(dce_design_ls, set_idx_1L_int = 1L, block_1L_int = 2L)
```

# Share work
The final step is to share our work with others in an online repository. Note, you will need to have previously supplied yvalues for `X` that correspond to a repository for which you have write persmissions to run this part of the code successfully.

```{r eval = FALSE}
Y <- share(X,
           obj_to_share_xx = dce_design_ls,
           fl_nm_1L_chr = ifelse(reproduce_1L_lgl,"DDD_pilot_dce_design_ls","CCC_pilot_dce_design_ls"),
           description_1L_chr = paste0("List object to help replicate pilot survey (output of dce_sa_design ",
